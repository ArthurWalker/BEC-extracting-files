...
def extract_data(excel_file,tab,extracted_lst,skiprow):
    ...
    if (tab == 'BE Workplaces main workbook'):
        ...
        print('Phase 4')
        extracted_df = temp_df[col_index_workbook]
    return extracted_df
...
def assign_task_Summary(seeep_path,file,folder):
    ...
    if 'Admin' in excel_file.sheet_names:
        worksheet['Admin'] = pd.read_excel(excel_file, 'Admin', nrows = 2,keep_default_na=False, header=None)
        lst_col_admin = ['Reference No.','Cat. ','Submitted By','Project Title','County','Approved Funding',]
        col_index_admin = find_column(worksheet['Admin'],lst_col_admin)
        admin_df = worksheet['Admin'][col_index_admin]
...
def assign_task_Overview(seeep_path,file,folder):
    ...
    if 'Technologies' in excel_file.sheet_names:
        lst_col_tech = ['SEAI Reference']
        # worksheet['Technologies'] = pd.read_excel(excel_file, 'Technologies',keep_default_na=False,header=None)
        # df_1_line =  worksheet['Technologies'].iloc[0]
        # col_index_tech = find_column(df_1_line,lst_col_tech)
        # tech_df = worksheet['Technologies'][col_index_tech]
